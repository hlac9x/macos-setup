    ...
    for _, workspace in ipairs(wezterm.mux.get_workspace_names()) do
        if workspace == active_workspace then
            table.insert(formatted_workspaces, wezterm.format(
                             {
                    {Attribute = {Intensity = "Bold"}}, {Text = workspace}
                }))
        else
...
return {
    ...
    font = wezterm.font("MesloLGS NF", {weight = "Medium"}),
    font_size = 16,
    ...
    keys = {
        ...
        {mods = "ALT", key = "0", action = action.ActivateTab(-1)}, -- Scroll
        {mods = "ALT", key = "u", action = action.ScrollByPage(-0.5)},
        {mods = "ALT", key = "d", action = action.ScrollByPage(0.5)}, -- Search
        {
            mods = "ALT",
            key = "/",
        ...
        -- Workspaces
        {
            mods = "ALT",
            key = "w",
            action = action.ShowLauncherArgs({flags = "WORKSPACES|FUZZY"})
        },
            ...
            action = wezterm.action_callback(
                function(window, pane)
                    ...
                    for _, project in ipairs(
                                          wezterm.read_dir(wezterm.home_dir ..
                                                               "/repo")) do
